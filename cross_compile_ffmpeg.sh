  ...
  cd $output_dir
    ...
    if [ "$bits_target" != "32" ]; then
      ...
      if [[ $ffmpeg_git_checkout_version == *"master"* ]]; then
        ...
        git am -3 "$work_dir/SVT-HEVC_git/ffmpeg_plugin/n4.4-0001-lavc-svt_hevc-add-libsvt-hevc-encoder-wrapper.patch"
      else
    ...
    if [[ $compiler_flavors != "native" ]]; then
      config_option+=" --enable-libflite" # don't work native build
    fi
    ...
    if [[ $enable_gpl == 'y' ]]; then
      config_options+=" --enable-gpl --enable-frei0r --enable-filter=frei0r --enable-librubberband --enable-libvidstab --enable-libx264 --enable-libx265 --enable-avisynth"
